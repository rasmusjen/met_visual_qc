...
def parse_raw_filename(name: str) -> Optional[RawFileId]:
    ...
    return RawFileId(site=m.group("site"), date=d, level=m.group("level"), file=m.group("file"))
...
def parse_header_filename(name: str) -> Optional[HeaderFileId]:
    ...
    return HeaderFileId(site=m.group("site"), valid_from=dt, level=m.group("level"), file=m.group("file"))
